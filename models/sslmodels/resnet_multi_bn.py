...
class predictor(nn.Module):
    def __init__(self, ch, bn_names=None):
        ...
        self.fc1 = nn.Linear(ch, ch * 2, bias=False)
        self.bn1 = batch_norm_multiple(nn.BatchNorm1d, ch * 2, bn_names)
        self.relu = nn.ReLU(inplace=True)
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x, bn_name=None, return_features=False):
    ...
    def forward(self, x, bn_name=None, return_features=False):
        return self._forward_impl(x, bn_name, return_features)
...
class resnet18_momentum(nn.Module):
    def __init__(self, pretrained=False, m=0.99, simsiam=False, **kwargs):
        super(resnet18_momentum, self).__init__()
        self.m = m
        self.encoder_q = resnet18(pretrained=pretrained, **kwargs)
        self.encoder_k = resnet18(pretrained=pretrained, **kwargs)
        self.out_dim = 512
    ...
    def forward(self, x, bn_name=None, swap=False):
        """
        Args:
                x: input tensor
                bn_name: name of the batchnorm layer
        """
        if swap:
            bsz = x.size()[0]
            out = torch.zeros(bsz, self.out_dim)
            out[::2] = self.encoder_q(x[::2], bn_name=bn_name)
            out[1::2] = self.encoder_k(x[1::2], bn_name=bn_name)
            return out
        else:
            return self.encoder_k(x, bn_name)
